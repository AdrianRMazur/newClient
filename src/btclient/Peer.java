...
public class Peer implements Runnable {
	...
	public boolean uploadToPeer() {
		...
		for (int j = 0; j < BTClient.torrentinfo.piece_hashes.length * 2 + 1; j++) {/*
																					 * limit
																					 * this
																					 * ,
																					 * so
			...
			if (id == Constants.HAVE_ID) { // Have
				...
				continue;
			} else if (id == Constants.REQUEST_ID) { // request, get the payload
														// bits
				try {
				...
				/*
				 * if(begin==435){ begin=16384; }
				 */

				if (BTClient.checkIfAvailable(index)) {
					BTClient.u = BTClient.u + length;
					System.arraycopy(BTClient.downloaded[index], begin, block,
							0, length);
					if (begin == 435) {
						begin = 16384;
					}

					Message piece = new Message(9 + length, (byte) 7, begin,
							id, block); // maybe one bit off here???
	...
	public void run() {
		if (uploadToPeer() == false) {
			System.out
					.println("There was an issue with uploading to one of the peers, but that's OK");
		}
	}
